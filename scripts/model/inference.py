...
def construct_conformers(data, model):
    ...
    new_pos = torch.zeros([data.batch.size(0), model.n_model_confs, 3]) # Initialize coord list
    ...
    for i, pair in enumerate(dihedral_pairs):
        ...
        y_cycle_check = [y_index in cycle for cycle in cycles] # Determine in cycle?
        ...
        # new graph
        p_coords = torch.zeros([4, model.n_model_confs, 3]) # dihedral coords?
